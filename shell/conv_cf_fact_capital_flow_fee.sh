#!/bin/sh
#####################################################################
# Script:  conv_cf_fact_capital_flow_fee.sh
# Project: Unicorn - AWS Glue Migration
# Created: 17-Oct-2019
# Developer: Sanjay Pancholi
#
# Usage:   conv_cf_fact_capital_flow_fee.sh date environment
#
#================================================================
#  Change Revision
...
#Script/job name:
JOB_NAME="conv_cf_fact_capital_flow_fee"
...
g_MSG="Required number of Arguments: ${REQRD_ARGUMENT_COUNT} and Not ${NUM_ARG} to ${RUN_SCRIPT} \\
      USAGE:   ${RUN_SCRIPT} batch-date environment \\
      EXAMPLE: ${RUN_SCRIPT} YYYY-MM-DD dev \\
      Exiting due to incorrect number of arguments."
Validate_Argument_Count $NUM_ARG $REQRD_ARGUMENT_COUNT "$g_MSG"

BATCH_DATE=$1
ENV=$2
g_MSG="BATCH_DATE=${BATCH_DATE}, ENV=${ENV}"
echo "$g_MSG"
...
#UNIX_Cmd_Exec
Log_Msg $C_INFO "Executing: ./${GLUE_JOB_TRIGGER_SCRIPT} ${JOB_NAME} ${JOB_ARGUMENTS}"
./${GLUE_JOB_TRIGGER_SCRIPT} ${JOB_NAME} ${JOB_ARGUMENTS}
