#!/bin/sh
#####################################################################
# Script:  ai_extracts.sh
# Project: Unicorn - AWS Glue Migration
# Created: 20-Feb-2020
# Developer: Amit Kheterpal
#
# Usage:   ai_extracts.sh date next_date env client_number aws_region
#
#================================================================
#  Change Revision
...
g_MSG="Required number of Arguments: ${REQRD_ARGUMENT_COUNT} and Not ${NUM_ARG} to ${RUN_SCRIPT} \\
      USAGE:   ${RUN_SCRIPT} batch-date environment client_Number aws_region \\
      EXAMPLE: ${RUN_SCRIPT} YYYY-MM-DD dev  client_num us-east-1 \\
...
g_MSG="BATCH_DATE=${BATCH_DATE}, NEXT_BATCH_DATE=${NEXT_BATCH_DATE}, ENV=${ENV}, CLIENT_NUMBER=${CLIENT_NUMBER}, AWS_REGION=${AWS_REGION}"
echo "$g_MSG"
...
JOB_ARGUMENTS="--BATCH_DATE=\"$BATCH_DATE\",--NEXT_BATCH_DATE=\"$NEXT_BATCH_DATE\",--ENV=\"$ENV\",--CLIENT_NUMBER=\"$CLIENT_NUMBER\""
