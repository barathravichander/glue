#!/bin/sh
#####################################################################
# Script:    conv_posval_fact_mv_multiplier.sh
# Project:   Unicorn - AWS Glue Migration
# Created:   24-Oct-2019
# Developer: Sanjay Pancholi
#
# Usage:   conv_posval_fact_mv_multiplier.sh date environment year aws-region
#
#================================================================
#  Change Revision
#     YYYY/MM/DD : <developer_name> : <Changes Made> : Bug no
#     2019/11/13 : pancholis : enhanced for usage of common.lib : NA (Enhancement)
#                              added 3rd param for AWS-Region
#     2019/11/29 : pancholis : enhanced for spark config param  : NA (Enhancement)
#================================================================
#####################################################################

#Script/job name:
JOB_NAME="conv_posval_fact_mv_multiplier"
...
g_MSG="Required number of Arguments: ${REQRD_ARGUMENT_COUNT} and Not ${NUM_ARG} to ${RUN_SCRIPT} \\
      USAGE:   ${RUN_SCRIPT} batch-date environment year aws-region \\
      EXAMPLE: ${RUN_SCRIPT} YYYY-MM-DD dev year us-east-1 \\
...
YEAR=$3
AWS_REGION=$4
g_MSG="BATCH_DATE=${BATCH_DATE}, ENV=${ENV}, YEAR=${YEAR}, AWS_REGION=${AWS_REGION}"
echo "$g_MSG"
...
#[29-Nov-2019] added config param for Spark "--config spark.yarn.executor.memoryOverhead=7G"
CONFIG_SPARK_MEM='spark.yarn.executor.memoryOverhead=7G'
JOB_ARGUMENTS="--BATCH_DATE=\"$BATCH_DATE\",--ENV=\"$ENV\",--YEAR=\"$YEAR\",--config=\"$CONFIG_SPARK_MEM\""
