#!/bin/sh
#####################################################################
# Script:  conv_cashflow_fact_capital_flow_fee.sh
# Project: Unicorn - AWS Glue Migration
# Created: 17-Oct-2019
# Developer: Sanjay Pancholi
#
# Usage:   conv_cashflow_fact_capital_flow_fee.sh date environment aws-region
#
#================================================================
#  Change Revision
#     YYYY/MM/DD : <developer_name> : <Changes Made> : Bug no
#     2019/10/31 : pancholis : enhanced for usage of common.lib : NA (Enhancement)
#     2019/11/01 : pancholis : moved common stuff to common.lib : NA (Enhancement)
#     2019/11/14 : pancholis : added 3rd param for AWS-Region   : NA (Enhancement)
#     2019/12/10 : motac : enhanced for spark config param  : NA (Enhancement)
#================================================================
#####################################################################

#Script/job name:
JOB_NAME="conv_cashflow_fact_capital_flow_fee"
...
g_MSG="Required number of Arguments: ${REQRD_ARGUMENT_COUNT} and Not ${NUM_ARG} to ${RUN_SCRIPT} \\
      USAGE:   ${RUN_SCRIPT} batch-date environment aws-region \\
      EXAMPLE: ${RUN_SCRIPT} YYYY-MM-DD dev us-east-1 \\
...
ENV=$2
AWS_REGION=$3
g_MSG="BATCH_DATE=${BATCH_DATE}, ENV=${ENV}, AWS_REGION=${AWS_REGION}"
echo "$g_MSG"
...
#[10-Dec-2019] added config param for Spark "--config spark.yarn.executor.memoryOverhead=7G"
CONFIG_SPARK_MEM='spark.yarn.executor.memoryOverhead=7G'
JOB_ARGUMENTS="--BATCH_DATE=\"$BATCH_DATE\",--ENV=\"$ENV\",--config=\"$CONFIG_SPARK_MEM\""
