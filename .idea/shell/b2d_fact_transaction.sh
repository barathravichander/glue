#!/bin/sh
#####################################################################
# Script:    b2d_fact_transaction.sh
# Project:   Unicorn - AWS Glue Migration
# Created:   20-Nov-2019
# Developer: Sanjay Pancholi
#
# Usage:   b2d_fact_transaction.sh date environment source-system-name aws-region
#
#================================================================
#  Change Revision
#     YYYY/MM/DD : <developer_name> : <Changes Made> : Bug no
#     2019/11/20 : pancholis : script copied from existing : NA (Enhancement)
#     2019/12/05 : pancholis : added forth param for source-sys name : NA (Enhancement)
#================================================================
#####################################################################

#Script/job name:
JOB_NAME="b2d_fact_transaction"
...
g_MSG="Required number of Arguments: ${REQRD_ARGUMENT_COUNT} and Not ${NUM_ARG} to ${RUN_SCRIPT} \\
      USAGE:   ${RUN_SCRIPT} batch-date environment source-system-name aws-region \\
      EXAMPLE: ${RUN_SCRIPT} YYYY-MM-DD dev BPSA us-east-1 \\
...
JOB_ARGUMENTS="--BATCH_DATE=\"$BATCH_DATE\",--ENV=\"$ENV\",--SOURCE_SYSTEM_NM=\"$SOURCE_SYSTEM_NM\""
