...
###############################
class DimInstrumentSourceDataProvider(BaseSourceDataProvider):

    def _get_initialized_obj(self):
        return SourceDataProviderOutput(None, C_H2B_CLIENT_NUMBER)

    def _transform_to_general_df(self, p_src_df):

        l_h2b_src_trans = DimInstrumentSrcStubTransformer()
        p_src_df = l_h2b_src_trans.transform(p_src_df)
        return p_src_df
...
class DimInstrumentSrcStubTransformer(BaseSrcStubTransformer):

    def _get_source_to_generic_clm_map(self):
        return {"security_adp_nbr": 'src_instrument_ref_nm'}
...
class DimInstrumentStubDataUpdater(object):

    def update(self, p_input_df,  for_dtl_instrument  = False):
        p_input_df = p_input_df.withColumn("instrument_type_cd", sf.lit('SEC')) \
            .withColumn("instrument_ref_type_cd", sf.lit('OT'))
        return p_input_df


class DimInstrumentStubAuxiliaryRecordCreator(InstrumentStubAuxiliaryRecordCreator):
