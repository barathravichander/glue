...
class GlueWithSecStubJob(GlueJobBase):

    src_df = None
    C_INSTRUMENT_STUB_JOB_PARAM = "INSTRUMENT_STUB"

    def _get_job_parameter_option_names(self):
        l_prop_list = super(GlueWithSecStubJob, self)._get_job_parameter_option_names()
        l_addtional_prop_list = [self.C_INSTRUMENT_STUB_JOB_PARAM]
    ...
    def _execute_job(self):
        ...
        if (instrument_stub.lower() == "y"):
            JobContext.job_config_map = JobConfigLoader.only_instrument_stub("inst_stub")

        else:
            JobContext.job_config_map = JobConfigLoader.only_instrument_stub("no_stub")
